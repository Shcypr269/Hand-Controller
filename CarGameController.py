...
def game():
    ...
    while cap.isOpened():
        ...
        if len(co) == 2:
            ...
            # Turn controls based on hand positions with progressive steering
            # Deadzone threshold - only turn if tilt is significant enough
            tilt_threshold = 20  # degrees
            
            # Determine turn direction and intensity based on base_angle
            turn_direction = 0  # 0 = straight, -1 = left, 1 = right
            
            # Check hand position patterns to determine direction
            if (co[0][0] > co[1][0] and co[0][1] > co[1][1]) or \
               (co[1][0] > co[0][0] and co[1][1] > co[0][1]):
                # Right hand lower = turn right
                turn_direction = 1
            elif (co[0][0] > co[1][0] and co[1][1] > co[0][1]) or \
                 (co[1][0] > co[0][0] and co[0][1] > co[1][1]):
                # Left hand lower = turn left
                turn_direction = -1
            
            # Apply progressive steering based on angle
            if turn_direction == 1 and base_angle > tilt_threshold:
                print(f"Turn right. Angle: {base_angle:.1f}")
                action = "right"
                if not drift:
                    keyinput.release_key('s')
                keyinput.release_key('a')
                keyinput.press_key('d')
                # Scale angle: small tilt = gentle turn, large tilt = sharp turn
                target_angle = min(90, base_angle)

            elif turn_direction == -1 and base_angle > tilt_threshold:
                print(f"Turn left. Angle: {base_angle:.1f}")
                action = "left"
                if not drift:
                    keyinput.release_key('s')
            ...
            # Smooth angle transition with adjustable sensitivity
            steering_angle = steering_angle * 0.8 + target_angle * 0.2
